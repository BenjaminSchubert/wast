...
class Step(Protocol):
    ...
    Steps can then be parametrized by the help of :py:func:`wast.parametrize`,
...
class StepWithSetup(Step, Protocol):
    ...
    .. tip::
        ...
        When running ``wast`` repeatedly, you can pass a ``--no-setup`` flag to
        avoid running those steps again and thus speedup your run.

        .. todo:: Link to a document that gives tips on how to run wast effectively

    :Examples:

        A setup method can be added on a step declaration:

        .. note:: A more complete pytest example is provided as :py:func:`wast.predefined.pytest`.
...
class StepWithArtifacts(Step, Protocol):
    ...
    :Examples:
        ...
        .. tip::
            ...
            This is what the provided :py:func:`wast.predefined.pytest` step
            does.
...
class StepWithCleanup(Step, Protocol):
    ...
    Sometimes, a step might write outside of the wast-managed cache, in which
    case it might be desirable for it to be able to clean that directory.

    This allows a step to hook on the ``wast --clean`` invocation to cleanup
    such files.
    ...
    :Examples:
        ...
        .. tip::
            ...
            :py:func:`wast.predefined.sphinx` does
